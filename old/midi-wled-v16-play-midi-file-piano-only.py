...
# Track active programs per channel
active_programs = {}
    ...
    with open_output(MIDI_OUTPUT_PORT) as output:
        for msg in midi.play():
            ...
            if msg.type == 'program_change':
                active_programs[msg.channel] = msg.program
                continue

            # Skip messages for non-piano/keyboard instruments
            if msg.type in ['note_on', 'note_off'] and active_programs.get(msg.channel, -1) not in range(0, 8):
                continue

            # Process piano/keyboard messages
...
print(f"Playing MIDI file '{MIDI_FILE}' and controlling LEDs (Only Piano/Keyboard Instruments). Press Ctrl+C to exit.")
try:
