...
def wait_for_user_input(expected_note):
    """Wait for user to play the expected note."""
    while True:
        for msg in input_port.iter_pending():
            if msg.type == 'note_on' and msg.velocity > 0:
                if msg.note == expected_note:
                    print(f"Correct! Played note: {msg.note}")
                    # Turn off the LED after the correct note is played
                    led_index = (expected_note - BASE_NOTE) * 2
                    if 0 <= led_index < NUM_LEDS:
                        set_led_color(led_index, [0, 0, 0])
                        set_led_color(led_index + 1, [0, 0, 0])
                    return True  # User played the correct note
                else:
    ...
    with open_output(MIDI_OUTPUT_PORT) as output:
        for msg in midi.play():
            ...
            if msg.type == 'note_on' and msg.velocity > 0:
                ...
                # Wait for user input to match the note
                wait_for_user_input(msg.note)
