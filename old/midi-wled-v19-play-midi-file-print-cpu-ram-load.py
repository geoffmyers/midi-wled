...
def monitor_system_usage():
    """Print CPU usage, RAM usage, and load average every second."""
    try:
        while True:
            ...
            ram_usage = memory.percent  # Percentage of RAM in use
            
            # Get system load average
            load_avg = psutil.getloadavg()  # Returns a tuple (1, 5, 15 minute load average)
            
            # Print the system status
            print(f"CPU Usage: {cpu_usage}% | RAM Usage: {ram_usage}% | Load Average (1m, 5m, 15m): {load_avg}")
            
            time.sleep(1)  # Sleep for 1 second before printing again
    except KeyboardInterrupt:
        pass  # Allow the script to exit cleanly

# Run the MIDI playback and system monitoring in separate threads
playback_thread = threading.Thread(target=play_midi_file, daemon=True)
system_monitor_thread = threading.Thread(target=monitor_system_usage, daemon=True)
