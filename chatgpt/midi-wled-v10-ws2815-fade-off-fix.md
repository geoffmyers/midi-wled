...
The observed issue arises because the `fade_out_led` function uses a loop with a delay (`time.sleep`), which executes sequentially within each thread. While threading can handle multiple tasks simultaneously, the `show()` function in the LED library updates the entire LED strip at once. If multiple threads attempt to update the strip concurrently, they may conflict or effectively serialize.

### Solution: Centralized Fade Control

To ensure LEDs fade in parallel:

1. **Use a centralized fade control loop**: Maintain a shared data structure (e.g., a dictionary) to track the fading states of all LEDs.
2. **Update all LEDs in a single loop**: Iterate over active fading LEDs at small intervals and calculate their brightness for the current step.
...
1. **Centralized Fading**:

   - A dictionary (`fading_leds`) tracks the LEDs and their fade states (color and steps remaining).
   - All fading LEDs are updated in parallel within a single `fade_loop`.

2. **Non-blocking Updates**:

   - The `fade_loop` ensures simultaneous fading without serializing LED updates.

3. **Efficient Updates**:
   - The `show()` function is called once per iteration to update all LEDs simultaneously.

This approach eliminates the serial fading issue and ensures smooth and simultaneous fading for all active LEDs.
